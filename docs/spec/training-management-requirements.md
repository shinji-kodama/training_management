...
- REQ-010: システムは 研修コースをPDF形式で出力 しなければならない
- REQ-011: システムは 研修提供先企業の情報を管理 しなければならない
- REQ-012: システムは 受講者の名前、メールアドレス、所属組織、役割タイプを管理 しなければならない
- REQ-013: システムは 研修コースの受講前提条件、ゴール、完了条件を管理 しなければならない
- REQ-014: システムは 面談記録をMarkdown形式で入力・保存 しなければならない
- REQ-015: システムは 定例会の研修記録を管理 しなければならない
...
- REQ-111: PDF出力時に教材リンクが含まれる場合、システムは リンクを有効な形式で出力 しなければならない
...
- NFR-002: システムは PDF出力処理を10秒以内で完了 しなければならない
- NFR-003: システムは 同時接続ユーザー数100人まで対応 しなければならない

### セキュリティ

- NFR-101: システムは パスワードをハッシュ化して保存 しなければならない
- NFR-102: システムは セッション管理による認証状態の維持 を実装しなければならない
...
- EDGE-005: PDF出力時にデータが不正な場合、システムはエラーメッセージを表示し出力を中止する
...
| REQ-011 | ストーリー8 | 企業管理 | 企業管理画面 | 企業管理テスト |
| REQ-012 | ストーリー9 | 受講者管理 | 受講者管理画面 | 受講者管理テスト |
| REQ-013 | ストーリー3 | 研修条件管理 | 研修管理画面 | 研修条件テスト |
| REQ-014 | ストーリー5 | Markdown記録 | 面談記録画面 | Markdown機能テスト |
| REQ-015 | ストーリー6 | 定例会記録 | 定例会画面 | 定例会記録テスト |
